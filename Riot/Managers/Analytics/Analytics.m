...
- (void)start
{
    NSDictionary *matomoConfig = [[NSUserDefaults standardUserDefaults] objectForKey:@"matomo"];

    // Check whether the user has enabled the sending of crash reports.
    // Opt out in case the side id equals -1
    if (RiotSettings.shared.enableCrashReport && [matomoConfig[@"siteId"] integerValue] != -1)
    {
        ...
        [[MatomoTracker shared] setCustomVariableWithIndex:3 name:@"Chosen Language" value:language];
        
        // Device and OS version
        [[MatomoTracker shared] setCustomVariableWithIndex:4 name:@"Device Model" value:deviceModel()];
        [[MatomoTracker shared] setCustomVariableWithIndex:5 name:@"OS Version" value:[[UIDevice currentDevice] systemVersion]];
